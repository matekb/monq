...
Job.prototype.save = function (callback) {
    ...
    if (this.data._id) {
      this.collection.replaceOne({ _id: this.data._id }, this.data, function (err, doc) {
        if (err) return callback(err);
        callback && callback(null, self);
      });
    } else {
      this.collection.insertOne(this.data, function (err, doc) {
        if (err) return callback(err);

        if (doc && self.data._id === undefined) {
            self.data._id = doc.insertedId;
        }
